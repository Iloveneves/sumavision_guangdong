...
typedef enum hiHA_ERRORTYPE_E
{
    ...
    HA_ErrorInsufficientResources = (HI_S32) 0x80001000,/**< 资源不够，创建设备失败 */

    HA_ErrorInvalidParameter = (HI_S32) 0x80001001, /**< 输入参数非法 */

    HA_ErrorStreamCorrupt = (HI_S32) 0x80001002, /**< 输入码流错误，解码失败 */

    HA_ErrorNotEnoughData = (HI_S32) 0x80001003,/**< 输入码流不够，退出解码 */

    HA_ErrorDecodeMode = (HI_S32) 0x80001004,/**< 解码模式不支持 */

	HA_ErrorNotSupportCodec = (HI_S32) 0x80001005,/**< codec dont support */
...
{
    HD_DEC_MODE_RAWPCM = 0,        /**< PCM 解码模式 */
    HD_DEC_MODE_THRU,              /**< 透传解码模式: SPIDF61937 pass-throgh only, such as AC3/DTS  */
    HD_DEC_MODE_SIMUL,             /**< PCM + 透传解码模式 */
...
/**
 * HA codec pcm 数据类型结构体定义
 *
 */
typedef struct hiHA_PCMMODETYPE_S
{
    HI_U32              u32DesiredOutChannels;/**<IN. 期望输出声道数目. 如果码流原始声道不等于
                                              期望输出声道，解码器自动进行down-mix 或up-mix 处理，使得输出
                                              声道等于u32DesiredOutChannels.  1: mono, 2 for stereo */
    HI_BOOL             bInterleaved;/**<IN. PCM 数据是否交织模式
                                     HI_TRUE:  交织模式: L/R/L/R...L/R/L/R
                                     HI_FALSE: 非交织模式: L/L/L......../R/R/R...... */
    HI_U32              u32BitPerSample;/**<IN. PCM 数据位宽,仅支持16 或24 数据位宽 16bit 位宽占用16bit 内存,
                                        24bit 位宽占用32比特内存.24bit PCM数据有效为在高位，低8 位补0 */
    HI_U32              u32DesiredSampleRate; /**<IN. 期望输出采样频率  */
    HI_HA_CHANNELTYPE_E enChannelMapping[HA_AUDIO_MAXCHANNELS];  /**<IN. 输出声道分布模式  */
} HI_HA_PCMMODETYPE_S;
...
typedef struct hiHADECODE_OPENPARAM_S
{
    ...
    HI_VOID *           pCodecPrivateData; /**<IN. 指向私有配置结构体的指针，如果解码器没有私有配置，设置为0 */
    HI_U32              u32CodecPrivateDataSize; /**<IN. 私有结构体大小, 如果解码器没有私有配置，设置为0 */
...
{
    HI_U32  u32PtsMs;     /**<IN. PTS (unit:MS) Note: HA decoder should discard this infomation */
    HI_U8  *pu8Data;      /**< IN/OUT. 输入码流指针，解码完成后解码器更新该指针  */
    HI_S32  s32Size;      /**< IN. 输入码流大小. unit: Byte */
...
{
    HI_S32 *ps32PcmOutBuf;       /**< IN. 指向PCM 解码输出的指针.
                                  note: 1) ps32PcmOutBuf must  be word32-aligned。
                                        2)  ps32PcmOutBuf=NULL when decoder working at HD_DEC_MODE_THRU mode */
    HI_U32  u32PcmOutBufSize;    /**< IN. PCM 解码输出缓冲区大小. unit:Byte
                                  note: u32PcmOutBufSize=0  when decoder working at  HD_DEC_MODE_THRU mode   */
    HI_S32 *ps32BitsOutBuf;      /**<IN. 指向透传解码输出的指针.
                                  note: 1) ps32BitsOutBuf must  be word32-aligned。
                                        2) ps32BitsOutBuf=NULL when decoder working at HD_DEC_MODE_RAWPCM mode  */
    HI_U32  u32BitsOutBufSize;   /**< IN. 透传(IEC61937) 解码输出缓冲区大小. unit:Byte
                                  note: u32BitsOutBufSize=0  when decoder working at HD_DEC_MODE_RAWPCM mode  */
    HI_U32  u32PcmOutSamplesPerFrame; /**<OUT. 解码Pcm 输出样点数 */
    HI_BOOL bInterleaved;             /**<OUT. PCM  解码交织模式   */
    HI_U32  u32BitPerSample;          /**<OUT. Bit per sample */
    HI_U32  u32BitsOutBytesPerFrame;  /**<OUT. 透传输出帧大小. unit: Byte */
    HI_U32  u32OutChannels;           /**<OUT. 输出声道数  */
    HI_U32  u32OutSampleRate;         /**< OUT. 输出采样频率 */
    HI_U32  u32OrgChannels;           /**<OUT. 码流原始声道数*/
    HI_U32  u32OrgSampleRate;         /**<OUT 码流原始采样频率 */
    HI_U32  u32BitRate;               /**<OUT 码流压缩率unit: bit/S */
...
typedef struct hiHA_DECODE_S
{
    ...
    /**< IN. 解码器标识，注意: 此标识作为解码器唯一身份标识，必须唯一而不能有重复
     */
    ...
    /**< OUT. 指向下个解码器设备的指针.  该成员变量由调用HA Codec 的客户端维护, HA 解码器开发
      者可以忽略该成员。
     */
    struct hiHA_DECODE_S *pstNext;

    /**< OUT. Pointer to dll symbol of the HA deocder.   该成员变量由调用HA Codec 的客户端维护, HA 解码器开发
      者可以忽略该成员。
     */
    HI_VOID  * pDllModule;
    ...
    /**< 解码器PCM  输出所需最大缓冲区size. 客户端需要根据该方法分配内存
        @param [in] hDecoder
            the decoder handle
    ...
    /**< 解码器透传 输出所需最大缓冲区size. 客户端需要根据该方法分配内存
        @param [in] hDecoder
            the decoder handle
        @param [in/out] pu32OutSizes
